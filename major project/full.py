...
header_title=Label(win,text="ITS BANK",font=('arial',50,'bold'),bg='white')
header_title.pack()

current_date=time.strftime('%d-%b-%Y')

header_date=Label(win,text=f'Date:{current_date}',font=('arial',20,'bold'),bg='white',fg='blue')
header_date.place(relx=0.8,rely=0.1)

footer_title=Label(win,text="By: Ansh Chauhan,shrey,preet pal",font=('arial',20,'bold'),bg='white')
footer_title.pack(side='bottom')

img=Image.open('C:\\Users\\ANSH\\OneDrive\\Desktop\\major project\\logo.jpg').resize((300,150))
...
def main_screen():
    ...
    def login_click():
        ...
        if uacn==0 and upass=='admin' and urole=='Admin':
            ...
            cur_obj.execute('select * from users where user_acno=? and user_pass=?',(uacn,upass))
    ...
    acn_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text="ACN")
    ...
    pass_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text="Pass")
    pass_label.place(relx=.3,rely=.2)

    pass_entry=Entry(frm,font=('arial',20,'bold'),bd=5,show='*')
    pass_entry.place(relx=.4,rely=.2)

    role_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text="Role")
    ...
    gen_captcha_label=Label(frm,font=('arial',20,'bold'),width=7,bg='white',fg='green',text=genrate_captcha())
    gen_captcha_label.place(relx=.45,rely=.4)

    captcha_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text="Captcha")
...
def welcome_admin_screen():
    ...
    def create_click():
        ...
        def open_acn():
            ...
            con_obj=sqlite3.connect(database='bank.sqlite')
            cur_obj=con_obj.cursor()
            cur_obj.execute('insert into users(user_pass, user_name, user_mob, user_email, user_bal, user_adhar, user_opendate) values(?,?,?,?,?,?,?)',(upass,uname,umob,uemail,ubal,uadhar,current_date))
            con_obj.commit()
            con_obj.close()
            ...
            try:
                gmail_con=gmail.GMail('ansh901538@gmail.com','pwnf iqkt xyzj wlsc')
                umsg=f'''Hello,{uname}
                Welcome to ITS Bank
                Your ACN is: {uacn}
                Your Pass is: {upass}
        ...
        name_entry=Entry(ifrm,font=('arial',18),bd=5)
        ...
        mob_entry=Entry(ifrm,font=('arial',18),bd=5)
        ...
        email_entry=Entry(ifrm,font=('arial',18),bd=5)
        ...
        adhar_entry=Entry(ifrm,font=('arial',18),bd=5)
    ...
    def delete_click():
        ...
        acn_entry=Entry(ifrm,font=('arial',18),bd=5)
    ...
    wel_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text="Welcome,Admin",fg='blue')
...
def forgot_password_screen():
    ...
    def get_password():
        ...
        if tup==None:
            ...
            try:
                print(111)
                gmail_con=gmail.GMail('ansh901538@gmail.com','fmjr bfac kniz jnvn')
                print(222)
                umsg=f'''Hello,{tup[0]}
                Welcome to ABC Bank
                Your Pass is: {tup[1]}
    ...
    email_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text="Email")
    email_label.place(relx=.3,rely=.2)

    email_entry=Entry(frm,font=('arial',20,'bold'),bd=5)
    email_entry.place(relx=.4,rely=.2)

    mob_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text="Mob")
...
def welcome_user_screen():
    ...
    default_image_path = 'C:\\Users\\ANSH\\OneDrive\\Desktop\\ducat\\pyhton\\project1\\default.png'


    if os.path.exists(f"{uacn}.png"):
        img=ImageTk.PhotoImage(Image.open(f'{uacn}.png').resize((200,200)),master=win)
    else:
        img=ImageTk.PhotoImage(Image.open(default_image_path).resize((200,200)),master=win)
    ...
    def update_photo():
        ...
        img=ImageTk.PhotoImage(Image.open(path).resize((200,200)),master=win)
        pic_label=Label(frm,image=img)
        pic_label.image=img
        pic_label.place(relx=.01,rely=.05)

    btn_update_pic=Button(frm,text="update pic",command=update_photo)
    btn_update_pic.place(relx=.1,rely=.35)
    ...
    def check_click():
        ...
        con_obj=sqlite3.connect(database='bank.sqlite')
        cur_obj=con_obj.cursor()
        cur_obj.execute('select user_bal, user_opendate, user_adhar from users where user_acno=?',(uacn,))
        tup=cur_obj.fetchone()
        con_obj.close()
    ...
    def update_click():
        ...
        def update_details():
            ...
            con_obj=sqlite3.connect(database='bank.sqlite')
            cur_obj=con_obj.cursor()
            cur_obj.execute('update users set user_name=?, user_pass=?, user_email=?, user_mob=? where user_acno=?',(uname,upass,uemail,umob,uacn))
            con_obj.commit()
            con_obj.close()
        ...
        con_obj=sqlite3.connect(database='bank.sqlite')
        cur_obj=con_obj.cursor()
        cur_obj.execute('select * from users where user_acno=?',(uacn,))
        tup=cur_obj.fetchone()
        con_obj.close()
        ...
        pass_entry=Entry(ifrm,font=('arial',18),bd=5)
    ...
    def deposit_click():
        ...
        def deposit_db():
            ...
            con_obj=sqlite3.connect(database='bank.sqlite')
            cur_obj=con_obj.cursor()
            cur_obj.execute('select user_bal from users where user_acno=?',(uacn,))
            ubal=cur_obj.fetchone()[0]
            con_obj.close()

            con_obj=sqlite3.connect(database='bank.sqlite')
            cur_obj=con_obj.cursor()
            cur_obj.execute('update users set user_bal=user_bal+? where user_acno=?',(uamt,uacn))
            con_obj.commit()
            con_obj.close()

            con_obj=sqlite3.connect(database='bank.sqlite')
            cur_obj=con_obj.cursor()
            cur_obj.execute('insert into txn(txn_acno,txn_type,txn_date,txn_amt,txn_updatebal) values(?,?,?,?,?)',(uacn,'Cr(+)',time.strftime('%d-%b-%Y %r'),uamt,ubal+uamt))  # Corrected column name
            con_obj.commit()
            con_obj.close()
    ...
    def withdraw_click():
        ...
        def withdraw_db():
            ...
            if ubal>=uamt:
                ...
                con_obj=sqlite3.connect(database='bank.sqlite')
                cur_obj=con_obj.cursor()
                cur_obj.execute('insert into txn(txn_acno,txn_type,txn_date,txn_amt,txn_updatebal) values(?,?,?,?,?)',(uacn,'Db(-)',time.strftime('%d-%b-%Y %r'),uamt,ubal-uamt))  # Corrected column name
                con_obj.commit()
                con_obj.close()
    ...
    def transfer_click():
        ...
        def transfer_db():
            ...
            con_obj=sqlite3.connect(database='bank.sqlite')
            cur_obj=con_obj.cursor()
            cur_obj.execute('select user_bal, user_email from users where user_acno=?',(uacn,))
            tup=cur_obj.fetchone()
            ubal=tup[0]
            uemail=tup[1]
            con_obj.close()
                ...
                if tup==None:
                    ...
                    try:
                        ...
                        if otp==uotp:
                            ...
                            con_obj=sqlite3.connect(database='bank.sqlite')
                            cur_obj=con_obj.cursor()
                            cur_obj.execute('insert into txn(txn_acno,txn_type,txn_date,txn_amt,txn_updatabal) values(?,?,?,?,?)',(uacn,'Db(-)',time.strftime('%d-%b-%Y %r'),uamt,ubal-uamt))
                            cur_obj.execute('insert into txn(txn_acno,txn_type,txn_date,txn_amt,txn_updatabal) values(?,?,?,?,?)',(toacn,'Cr(+)',time.strftime('%d-%b-%Y %r'),uamt,ubal+uamt))
        ...
        amt_label=Label(ifrm,font=('arial',20,'bold'),bg='white',text="Amount",fg='blue')
        amt_label.place(relx=.25,rely=.4)
    ...
    wel_label=Label(frm,font=('arial',20,'bold'),bg='light gray',text=f"Welcome,{uname}",fg='blue')
    ...
    transfer_btn=Button(frm,command=transfer_click,width=15,text='transfer amt',font=('arial',20,'bold'),bd=5,bg='powder blue')
